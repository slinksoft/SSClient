...
extern "C" { 
    ...
    DLLEXP kailleraInit(){ 
		...
		//Open File
		config.open("supraslinkclient.ini", ios::in);
		...
		//Server Lists
		strcpy(anti3DServerList.host, "3.85.254.166");
    ...
    DLLEXP kailleraSetInfos(kailleraInfos *infosp){
		...
		strcpy(userlistCommands[41].commands, "/tempmod ");
		strcpy(userlistCommands[42].commands, "/tempmod ");
		strcpy(userlistCommands[43].commands, "/tempmod ");
		strcpy(userlistCommands[44].commands, "/tempmod ");
		strcpy(userlistCommands[45].commands, "/tempmod ");
		strcpy(userlistCommands[46].commands, "/tempmod ");
		strcpy(userlistCommands[47].commands, "/tempmod ");
		strcpy(userlistCommands[48].commands, "/tempmod ");
		strcpy(userlistCommands[49].commands, "/tempmod ");
		strcpy(userlistCommands[50].commands, "/tempmod ");
    ...
    DLLEXP kailleraSelectServerDialog(HWND parent){ 
		...
		wcex.hbrBackground  = (HBRUSH)COLOR_BTNFACE + 0;
		wcex.lpszMenuName   = NULL;
		wcex.lpszClassName  = "SupraSlinkClient";
		wcex.hIconSm        = NULL;
		...
		case WM_KEYDOWN:{
			switch(LOWORD(wParam)){
				case VK_RETURN:{
					...
					if(myGameID != -1 && imOwner == true && EmulinkerXValue == BST_CHECKED){
						data[0] = 'd';
		...
		case WM_COMMAND:{
			switch(wParam){
				case BN_CLICKED:{
					...
					else if(hwndCtl == chkEmulinkerX){
						EmulinkerXValue = SendMessage(chkEmulinkerX, BM_GETCHECK, 0, 0);
					}
...
void createEditFavoritesWindow(){
	...
	frmEditFavorites = CreateWindow("SupraSlinkClient", "Edit Favorite Entry", formOtherProperties, b.left + 110, b.top + 200, 330, 150, form1, NULL, hInstance, NULL);
	//IP
...
void createCommentsWindow(){
	...
	frmComments = CreateWindow("SupraSlinkClient", "Any Comments?", formOtherProperties, b.left + 110, b.top + 200, 330, 80, form1, NULL, hInstance, NULL);
	//Subject
...
void createPMWindow(){
	...
	frmPM = CreateWindow("SupraSlinkClient", "Enter a message:", formOtherProperties, b.left + 110, b.top + 200, 330, 80, form1, NULL, hInstance, NULL);
	//Subject
...
void createAwayWindow(){
	...
	frmAway = CreateWindow("SupraSlinkClient", "Create Away Message", formOtherProperties, b.left + 110, b.top + 80, 440, 340, form1, NULL, hInstance, NULL);
	//Subject
...
int saveConfig(){
	ofstream config("supraslinkclient.ini", ios::out);
	char temp[2048];

	if(!config.good()){
		MessageBox(form1, "Error occurred while saving to supraslinkclient.ini", "Error Saving!", NULL);
		return 1;
	}

	config << "[SupraSlinkClient Config File]" << "\n\n";
...
void showOptions(char show){
	...
	//Emulinker Options
		...
		ShowWindow(chkEmulinkerX, SW_SHOW);
...
void joinGameNotification(unsigned short position, int slot){
	...
	if(userID != myUserID){
		...
		saveGameroomLog("####################\r\n");
		if(imOwner == true && EmulinkerXValue == BST_CHECKED){
			//Max Users
			GetWindowText(txtMaxUsers, maxUsersG, GetWindowTextLength(txtMaxUsers) + 1);
			if(atoi(maxUsersG) < 1 || atoi(maxUsersG) > 100){
				wsprintf(maxUsersG, "%i", 16);
